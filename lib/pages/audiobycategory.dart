...
import '../model/getaudiobycategorymodel.dart';

class AudiobyCategory extends StatefulWidget {
  final String categoryid, userid;
  const AudiobyCategory(
      {super.key, required this.categoryid, required this.userid});

  @override
  State<AudiobyCategory> createState() => AudiobyCategoryState();
}

class AudiobyCategoryState extends State<AudiobyCategory>
    with SingleTickerProviderStateMixin {
  TabController? tabController;
  late PagewiseLoadController<Result> audiobycategoryController;
  int pageno = 1;
  int tabindex = 1;
  ...
  void initState() {
    ...
    tabController =
        TabController(length: Constant().searchItemTab.length, vsync: this, initialIndex: 0);
    getApi();
    tabController?.addListener(handleTabIndex);
  }

  void handleTabIndex() async {
  ...
  Future<List<Result>> fetchNewData(int? nextPage) async {
    final audiolistprovider =
        Provider.of<AudiobyCategoryProvider>(context, listen: false);
    pageno = (nextPage ?? 0) + 1;
    debugPrint("Pageno =>$pageno");
    debugPrint("tabindex =>$tabindex");
    await audiolistprovider.getAudiobycategory(
        widget.categoryid, tabindex.toString(), pageno.toString());
    return audiolistprovider.getaudiobycategoryModel.result ?? [];
  }

  @override
  void dispose() {
    tabController?.removeListener(handleTabIndex);
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: PagewiseGridView<Result>.count(
            ...
            pageLoadController: audiobycategoryController,
            mainAxisSpacing: 5,
            crossAxisSpacing: 5,
